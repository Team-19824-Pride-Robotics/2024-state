...
public class teleop_v2 extends LinearOpMode {
    ...
    public static double out_half = 0.12;
...
//left bumper sends everything to intake positions
            ...
            if (!liftControl) {
                ...
                if(arm_encoder_position > 300 && rightrigger <.15) {
                    target = lift_intake;
                }

                else if (arm_encoder_position > 300 && rightrigger > .15) {
                    bPosx = bucket_low;
                    target = lift_low;
                    pincer_left.setPosition(left_closed);
                    pincer_right.setPosition(right_closed);
